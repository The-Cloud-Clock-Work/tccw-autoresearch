...
<!-- DETERMINISTIC: validate the target path -->
```bash
...
Verify that both `autoresearch` and `claude` are available:

```bash
command -v autoresearch && autoresearch --version
command -v claude && claude --version
```

If `autoresearch` is missing, tell the user:
> autoresearch CLI is not installed. Run `pip install -e .` from the tccw-autoresearch directory first.

If `claude` is missing, tell the user:
> Claude Code is a **hard dependency** — it's the AI agent that actually edits code.
...
## Step 4: Run `autoresearch init` (AI-JUDGMENT)

Run `autoresearch init` in the target repo to create the `.autoresearch/` directory structure and default agent profiles:

```bash
cd <repo-path> && autoresearch init --headless
```

If `.autoresearch/config.yaml` already exists, inform the user and ask whether to add a new marker or reconfigure.
...
**Important:** Offer to run the metric command right now to establish the baseline value. If the user agrees, run it in the target repo, extract the number, and use it as `baseline`.
...
## Step 12: Measure Baseline (AI-JUDGMENT)

Run the metric command in the target repo to establish the baseline:

```bash
cd <repo-path> && <metric.command> | <metric.extract>
```
